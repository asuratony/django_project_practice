...
# https://gitee.com/itcastitheima/meiduo_35.git
...
SECRET_KEY = '4ll7&rd5@5u%j)ueo88y2=g5v6q81voz77hbmadqdesulu610g'
...
# 我们的子应用都放在了 apps 包中 ,我们只需要告知系统去哪里找就可以
import sys
#sys.path 列表
sys.path.insert(0,os.path.join(BASE_DIR,'apps'))
# 我们使用 子应用的时候 不需要在添加 apps 因为系统已经知道了
...
REST_FRAMEWORK = {
    ...
    'DEFAULT_AUTHENTICATION_CLASSES': (
        ...
        'rest_framework.authentication.SessionAuthentication',  # 如果用户没有传递token,则采用session认证
...
# 1.我们要替换系统默认的User 需要通过 AUTH_USER_MODEL 来实现
#2. 值的语法形式为: 子应用.模型类名 只能有一个点(.)
...
# FastDFS
FDFS_URL = 'http://192.168.96.128:8888/'  # 访问图片的路径域名 ip地址修改为自己机器的ip地址
...
#  搜索 --> haystack --> elasticsearch

# Haystack
HAYSTACK_CONNECTIONS = {
    'default': {
        'ENGINE': 'haystack.backends.elasticsearch_backend.ElasticsearchSearchEngine',
        'URL': 'http://192.168.96.128:9200/',  # 此处为elasticsearch运行的服务器ip地址，端口号固定为9200
...
ALIPAY_URL = "https://openapi.alipaydev.com/gateway.do"  #沙箱的支付网关
ALIPAY_DEBUG = True
...
"""
1.所有以 www.meiduo.site 开头的网址 都是 静态资源

2.所有以 api.meiduo.site:8000 开头的网址 都是 动态资源

3.Nginx 只要处理静态业务业务逻辑
...
静态资源:
    1. 收集django的静态资源
    2. 让nginx引导我们的请求到 front文件

    在浏览器中输入 http://www.meiduo.site/ --> Nginx  --> Nginx引导到front文件夹就可以了

动态资源:
    用户在浏览器中输入 api.meiduo.site:8000 --> Nginx (nginx 不能处理动态业务逻辑)
     --> uwsgi (nginx会把请求(socket)转发给uwsgi) --> Django
"""
